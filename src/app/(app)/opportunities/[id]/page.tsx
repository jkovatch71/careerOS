...
export default async function OpportunityIntelligencePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [opportunityResult, analysesResult, outreachResult, followUpsResult] = await Promise.all([
    supabase.from("opportunities").select("*, employer:companies!opportunities_company_id_fkey(id, name, website, industry), recruiting_firm:companies!opportunities_recruiting_firm_id_fkey(id, name), primary_contact:contacts!opportunities_primary_contact_id_fkey(id, name, title, email, linkedin_url), recruiter:contacts!opportunities_recruiter_contact_id_fkey(id, name, title, email, linkedin_url)").eq("id", id).maybeSingle(),
    supabase.from("ai_analyses").select("*, resume:resumes(id, name)").eq("opportunity_id", id).order("created_at", { ascending: false }),
  ...
  const hasLoadError = Boolean(opportunityResult.error || analysesResult.error || outreachResult.error || followUpsResult.error);
  ...
  return <div className="mx-auto max-w-7xl">
    ...
    <div className="mt-5 flex flex-col justify-between gap-4 border-b pb-6 sm:flex-row sm:items-end"><div><div className="flex flex-wrap items-center gap-2"><span className="rounded-full border bg-muted/30 px-2.5 py-1 text-xs">{opportunityStageLabel(opportunity.stage)}</span>{opportunity.fit_score !== null ? <span className="rounded-full border px-2.5 py-1 font-mono text-xs">Fit {opportunity.fit_score}</span> : null}</div><h1 className="mt-3 text-2xl font-semibold tracking-tight md:text-3xl">{opportunity.role_title}</h1><p className="mt-2 text-sm text-muted-foreground">{opportunity.employer?.name ?? "Company not set"}{opportunity.employer?.industry ? ` · ${opportunity.employer.industry}` : ""}</p></div><div className="grid grid-cols-2 gap-x-8 gap-y-2 text-sm sm:text-right"><div><p className="text-xs text-muted-foreground">Compensation</p><p className="mt-1">{opportunity.compensation ?? "—"}</p></div><div><p className="text-xs text-muted-foreground">Applied</p><p className="mt-1">{formatDate(opportunity.applied_at)}</p></div></div></div>
    ...
    <section className="mt-6 grid gap-4 lg:grid-cols-[1.45fr_1fr]">
      ...
      <Card><CardHeader><CardTitle className="text-base">Relationships</CardTitle><CardDescription>People and firms influencing this opportunity.</CardDescription></CardHeader><CardContent className="space-y-4"><Relationship icon={Building2} label="Employer" name={opportunity.employer?.name} detail={opportunity.employer?.website} /><Relationship icon={UserRound} label="Primary contact" name={opportunity.primary_contact?.name} detail={opportunity.primary_contact?.title ?? opportunity.primary_contact?.email} /><Relationship icon={BriefcaseBusiness} label="Recruiting firm" name={opportunity.recruiting_firm?.name} /><Relationship icon={UserRound} label="Recruiter" name={opportunity.recruiter?.name} detail={opportunity.recruiter?.title ?? opportunity.recruiter?.email} /></CardContent></Card>
    </section>
